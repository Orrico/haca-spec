...
A special class of state-write payload is the **Structural Consolidation Proposal**: an intent payload addressed to the MIL that declares a body of accumulated semantic knowledge as a candidate for structural promotion — to be committed to the Entity Store via the Endure Protocol. A Structural Consolidation Proposal is produced by the CPE during an active session, when the entity determines that accumulated semantic content is sufficiently significant to become part of its identity baseline rather than remain as episodic memory. The proposal is submitted to the SIL for authorization before reaching the MIL. The SIL's response is profile-dependent: under HACA-Core, a proposal is rejected unless accompanied by explicit Operator authorization; under HACA-Evolve, a proposal within the entity's current operational authorization scope is approved and queued as an Endure event for the next Sleep Cycle. A rejected proposal is logged; the semantic content remains in the Memory Store as mnemonic content.
...
- **Inference Drift** — an intent payload produced by the cognitive engine contradicts verified persisted state in the memory layer. **Detected by the CPE** per Cognitive Cycle, before the payload is dispatched. The CPE signals the SIL; the SIL classifies and responds.
...
**Structural Consolidation Proposal** — a special class of state-write intent payload produced by the CPE during an active session. Declares accumulated semantic knowledge as a candidate for structural promotion via the Endure Protocol. Submitted to the SIL for authorization: approved under HACA-Evolve within operational authorization scope; requires explicit Operator authorization under HACA-Core. Approved proposals are queued as Endure events; rejected proposals leave the semantic content in the Memory Store as mnemonic content.
