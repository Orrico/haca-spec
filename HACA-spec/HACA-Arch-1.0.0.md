...
The CPE maintains a **Volatile Diagnostic Buffer** that retains the full `<internal_voice>` output for the duration of the current Cognitive Cycle only. If the cycle closes with a Nominal Heartbeat state, the buffer is discarded without being written to any persistent store. If the session token is revoked due to an anomaly — Watchdog timeout, Heartbeat Vital Check concluding Critical, or any other SIL-initiated halt caused by a detected integrity or behavioral violation — the SIL emits a buffer lock signal to the CPE via the Health Flow. The CPE executes the lock internally — suspending the discard path — and signals confirmation to the SIL. The SIL then instructs the MIL to persist the locked buffer as a sealed diagnostic attachment appended to the cycle's integrity log entry. The long-term Memory Store is unaffected; the diagnostic record is written exclusively to the static integrity log. Operational revocations — such as a controlled session close triggered by a Context Window Critical signal — do not trigger VDB lock; the buffer is discarded normally. This mechanism preserves the privacy and mnemonic purity of normal operation while ensuring that the deliberation trace of a compromised cycle remains available for forensic analysis of prompt injection or behavioral anomalies.
...
The SIL receives health signals from every component, evaluates them against defined integrity criteria, and — in Degraded conditions — issues corrective signals to the affected component. Each component is responsible for executing its own corrective actions within its authority when signaled by the SIL; the SIL does not directly manipulate another component's internal state. Drift detection is distributed: the CPE detects Inference Drift and the MIL detects Semantic Drift, each signaling the SIL within their respective operational windows. The SIL detects Identity Drift and Evolutionary Drift directly through structural hash comparison and integrity chain analysis. In all four cases, classification and response are exclusively the SIL's responsibility. When the condition exceeds the SIL's correction authority, it escalates to the Operator via the Operator Channel. The SIL operates reactively by default and retains the authority to intervene proactively in the following predefined high-risk conditions: (1) the SIL detects, before an intent payload is dispatched, that the target skill's manifest is inconsistent or that the declared capability boundaries would be exceeded — the SIL may block the dispatch before the EXEC receives the payload; (2) the number of consecutive Degraded states within a single session exceeds a defined accumulation threshold, suggesting systemic deterioration before a Critical Vital Check is reached — the SIL may revoke the session token preemptively. The specific thresholds for each condition are defined at implementation time or by the active Cognitive Profile. When escalating to the Operator, the SIL uses the Operator Channel directly — bypassing the CPE — because the CPE may be the source of the detected anomaly.
...
Each component maintains a **Reciprocal SIL Watchdog**: a local silence counter that increments whenever the expected SIL poll does not arrive within the defined interval. If the counter reaches its threshold, the component registers a SIL silence event locally but does not immediately escalate. Escalation to the Operator requires dual confirmation: a second distinct component must independently reach its own threshold within the same silence window. The first component to confirm registers the condition; the second component to confirm activates the Operator Channel directly, recording both detecting components, the silence duration, and the last known operational state of the SIL. This dual-confirmation requirement prevents false positives from transient timing or network conditions. If only one component ever reaches its threshold — the other recovers before confirming — no escalation is issued and both components log the event locally for the next Heartbeat Vital Check. This is the only condition under which a component other than the SIL may contact the Operator directly. The Reciprocal SIL Watchdog threshold and silence window are defined at implementation time. **Exception — Sleep Cycle**: during the Sleep Cycle, only the MIL and the SIL are active; the CPE and EXEC are not present to provide a second confirmation. If the MIL's Reciprocal SIL Watchdog reaches its threshold during the Sleep Cycle, the MIL is authorized to escalate unilaterally to the Operator Channel without waiting for dual confirmation. The MIL records the unilateral escalation, the Sleep Cycle stage at which the silence was detected, and the last known SIL operational state.
...
**Reciprocal SIL Watchdog** — a local silence counter maintained by each component that detects SIL polling gaps. Escalation requires dual confirmation: a second distinct component must independently reach its own threshold within the same silence window before the Operator Channel is activated. The first component registers the condition; the second triggers escalation. The only condition under which non-SIL components may contact the Operator directly.
